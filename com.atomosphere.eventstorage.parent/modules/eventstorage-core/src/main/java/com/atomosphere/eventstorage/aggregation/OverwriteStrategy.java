...
public class OverwriteStrategy implements EventAggregationStrategy {
	...
	@Override
	public void aggregate(IgniteCache<Binary, byte[]> eventCache, IgniteCache<Binary, byte[]> snapshotCache, byte[] key, int registered, int aggregated) {
		Event event = new Event().unmarshal(eventCache.get(Binary.of(new EventVersion().withKey(key).withVersion(registered))));
		snapshotCache.put(Binary.of(key), event.value);
	}
